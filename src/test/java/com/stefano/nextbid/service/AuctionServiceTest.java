...
class AuctionServiceTest {
    ...
    void createAuctionWhileAuthenticatedShouldSuccess() {
        CreateAuctionBody body = new CreateAuctionBody(
                "title", "description", 10.0, Instant.parse("3025-01-01T12:00:50.00Z")
        );
        Auction mappedAuction = new Auction(
                body.title(), body.description(), body.dueDate(), body.initialBid(), new User(1), null
        );
    ...
    void getAllAuctionWhenThereAreAuctionsShouldReturnList() {
        ...
        databaseAuctions.add(new Auction(
                "title1", "description1", Instant.now().plus(1, ChronoUnit.DAYS),
                10.0, new User(1), null)
        );
        databaseAuctions.add(new Auction(
                "title2", "title1", Instant.now().plus(1, ChronoUnit.DAYS),
                10.0, new User(1), null)
        );
    ...
    void updateAuctionByIdWithValidIdShouldSuccess() {
        ...
        Auction auction = new Auction(
                "title", "desc", Instant.now().plus(1, ChronoUnit.DAYS),
                10.0, new User(100), null
        );
        when(sessionManager.isAuthenticated()).thenReturn(true);
