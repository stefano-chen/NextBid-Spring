...
class BidServiceTest {
    ...
    void createBidWithInvalidAuctionIdShouldThrow() {
        ...
        assertThrows(InvalidIdException.class, () -> bidService.createBid(-10, new CreateBidBody(100)));
    }

    @Test
    void createBidForExpiredAuctionShouldThrow() {
        Auction auction = new Auction("auction", "desc", Instant.now().minus(1, ChronoUnit.DAYS), 10, new User(1), null);

        when(sessionManager.isAuthenticated()).thenReturn(true);
        when(auctionRepository.findById(1)).thenReturn(Optional.of(auction));

        assertThrows(AuctionClosedException.class, () -> bidService.createBid(1, new CreateBidBody(10)));
    }

    @Test
    void createBidWithLowBidThanLastBidShouldThrow() {
        Auction auction = new Auction("auction", "desc", Instant.now().plus(1, ChronoUnit.DAYS), 10, new User(1), null);
        ...
        assertThrows(AmountTooLowException.class, () -> bidService.createBid(1, new CreateBidBody(10)));
    }
